...
ifeq (cleveldb,$(findstring cleveldb,$(MARKET_BUILD_OPTIONS)))
  build_tags += gcc
endif
...
build-load-test:
	go build -tags tmload -o build/market-load-test ./tests/tm-load-test/market-load-test/
...
.PHONY: in-docker
in-docker:
	docker build -t market-dev-utils ./dev/tools -f dev/tools/devtools.Dockerfile
	docker run -i --rm \
		-v ${PWD}:/go/src/github.com/pendulum-labs/market:delegated \
		--mount source=dev-tools-cache,destination=/cache/,consistency=delegated market-dev-utils bash -x -c "\
		$(ARGS)"
